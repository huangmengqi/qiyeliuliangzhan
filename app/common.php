...
function send_email($templateVar, $emailNumbers, $templateParam = [])
{
    ...
    if (PHP_VERSION > 5.5) {
        $email_object  = new \app\common\api\SendMailer6();
    } else {
        $email_object  = new \app\common\api\SendMailer5();
    }
    
    $result = [];
    foreach($emailNumbers as $email) {
        $result[] = $email_object->send($email, $template['email_title'], $template['content'], $template['fromname'] ? $template['fromname'] : '', $template['file'] ? $template['file'] : '');
    }
    return $result;
}
